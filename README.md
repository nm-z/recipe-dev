...
Inference blocks across the selected devices. An empty split is measured: each block joins the current device while its parameters and carried state fit that device's free memory, and starts the next device when they do not. A block no remaining device can hold is reported, so a measured placement never plans an allocation its device cannot take. The CPU is selectable last so a placement can end on the host. A split names the blocks each device takes instead. Every range runs as its own tape on its device and the stream hops between them, so the output equals a single-device run.
