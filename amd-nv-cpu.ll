...
br label %loop
loop:
%lane = phi i32 [ 0, %entry ], [ %next, %store ]
%fragment = phi <RECIPE_REGISTER_M x double> [ zeroinitializer, %entry ], [ %updated, %store ]
%more = icmp ult i32 %lane, RECIPE_REGISTER_M
br i1 %more, label %check, label %done
check:
%coordinate = add i32 %output.m.base, %lane
%inside = icmp ult i32 %coordinate, %tile.m
br i1 %inside, label %load, label %store
load:
%index = call i32 @contraction_a_index(i32 %k, i32 %coordinate, i32 %tile.m, i32 %tile.k)
%ptr = getelementptr [0 x double], ptr addrspace(3) @contraction_tile, i32 0, i32 %index
%value = load double, ptr addrspace(3) %ptr, align 8
br label %store
store:
%selected = phi double [ 0.0, %check ], [ %value, %load ]
%updated = insertelement <RECIPE_REGISTER_M x double> %fragment, double %selected, i32 %lane
%next = add i32 %lane, 1
...
loop:
%lane = phi i32 [ 0, %entry ], [ %next, %store ]
%fragment = phi <RECIPE_REGISTER_N x double> [ zeroinitializer, %entry ], [ %updated, %store ]
%more = icmp ult i32 %lane, RECIPE_REGISTER_N
br i1 %more, label %check, label %done
check:
%coordinate = add i32 %output.n.base, %lane
%inside = icmp ult i32 %coordinate, %tile.n
br i1 %inside, label %load, label %store
load:
%base = mul i32 %tile.m, %tile.k
%local = call i32 @contraction_b_index(i32 %k, i32 %coordinate, i32 %tile.n, i32 %tile.k)
%index = add i32 %base, %local
%ptr = getelementptr [0 x double], ptr addrspace(3) @contraction_tile, i32 0, i32 %index
%value = load double, ptr addrspace(3) %ptr, align 8
br label %store
store:
%selected = phi double [ 0.0, %check ], [ %value, %load ]
%updated = insertelement <RECIPE_REGISTER_N x double> %fragment, double %selected, i32 %lane
%next = add i32 %lane, 1
br label %loop, !llvm.loop !0
done:
...
br i1 %local.lane, label %local.owner.check, label %shared.chunk.loop
local.owner.check:
...
local.product.step:
%local.a.index = urem i32 %local.product, RECIPE_REGISTER_ROWS
%local.b.index = udiv i32 %local.product, RECIPE_REGISTER_ROWS
...
%store.register = phi i32 [ 0, %accumulate.done ], [ %store.register.next, %store.next ] %store.more = icmp ult i32 %store.register, RECIPE_REGISTER_COUNT br i1 %store.more, label %store.check, label %output.done
store.check: %store.output.m.raw = call i32 @contraction_output_m(i32 %output.id, i32 %store.register, i32 %m.lanes) %store.output.n.raw = call i32 @contraction_output_n(i32 %output.id, i32 %store.register, i32 %m.lanes) %store.register.valid = call i1 @contraction_output_register_valid(i32 %store.register)
...
i32 %rows, i32 %from, i32 %heads, i32 %channels, i32 %tile.m, i32 %tile.n, i32 %tile.k, i64 %threads,
...
call void @grid_barrier(i64 %threads) br label %row.loop row.loop:
...
gradient.store.loop:
%gradient.store.register = phi i32 [ 0, %gradient.product.done ], [ %gradient.store.register.next, %gradient.store.next ] %gradient.store.more = icmp ult i32 %gradient.store.register, RECIPE_REGISTER_COUNT br i1 %gradient.store.more, label %gradient.store.check, label %gradient.output.done
...
%previous.store.register = phi i32 [ 0, %previous.load.ready ], [ %previous.store.register.next, %previous.store.next ] %previous.store.more = icmp ult i32 %previous.store.register, RECIPE_REGISTER_COUNT br i1 %previous.store.more, label %previous.store.check, label %previous.output.done
previous.store.check: %previous.store.output.m.raw = call i32 @contraction_output_m(i32 %previous.output.id, i32 %previous.store.register, i32 %previous.m.lanes) %previous.store.output.n.raw = call i32 @contraction_output_n(i32 %previous.output.id, i32 %previous.store.register, i32 %previous.m.lanes) %previous.store.register.valid = call i1 @contraction_output_register_valid(i32 %previous.store.register)
...
i32 %previous.tile.m, i32 %previous.tile.n, i32 %previous.tile.k, i64 %threads ) call void @grid_barrier(i64 %threads) %projection.next = add i32 %projection.gate, 1 br label %projection.loop
