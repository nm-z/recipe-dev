...
fn compile_nvidia(manifest: &str, out: &PathBuf, os: &str, schedule: Schedule) -> BuildResult<()> {
	...
	println!("cargo:rustc-env=RECIPE_NV_PTX_GENERATOR={}", platform(manifest, "nvidia-ptx-generator", os)?);
	Ok(())
}
...
fn main() -> BuildResult<()> {
	...
	let nvidia = native
		&& installed("nvidia-compiler")?
		&& installed("nvidia-ptx-generator")?
		&& toolkit.as_ref().is_some_and(|toolkit| toolkit.device_library.exists());
	if amd {
