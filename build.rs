...
"#;
/// The block dots: one 16-value slice of a stored block against the
/// activations as they are, staged channel-interleaved in the local store in
/// the model type. Each
/// code multiplies the activation in the state exactly, the products add in
/// the state, and the block's step scales the sum once, so nothing about the
/// activation is quantized: the only quantization is the one the load kernel
/// did. The helpers are written in the template's placeholders and serve
/// every backend.
