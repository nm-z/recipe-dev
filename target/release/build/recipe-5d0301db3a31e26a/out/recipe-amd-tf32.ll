...
load.ready:
call void @llvm.amdgcn.s.barrier()
call void @contraction_vector_accumulate(ptr addrspace(5) %sums, i1 %lane.active, i1 %method.store, i32 %output.id, i32 %lane.k, i32 %k.lanes, i32 %output.lane, i32 %lanes, i32 %output.m.base, i32 %output.n.base, i32 %m.count, i32 %n.count, i32 %k.count, i32 %tile.m, i32 %tile.n, i32 %tile.k)
...
%store.bias.base = mul i32 %out.channels, %terms %store.bias.index = add i32 %store.bias.base, %store.channel %store.bias.ptr = getelementptr inbounds float, ptr addrspace(1) %weights, i32 %store.bias.index %store.bias = load float, ptr addrspace(1) %store.bias.ptr, align 4 %store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr addrspace(5) %sums, i32 0, i32 %store.register %store.sum.wide = load float, ptr addrspace(5) %store.sum.ptr, align 4 %store.sum = call float @recipe.encode(float %store.sum.wide)
...
%gradient.store.ptr = getelementptr inbounds float, ptr addrspace(1) %gradient.destination, i32 %gradient.store.index %gradient.store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr addrspace(5) %sums, i32 0, i32 %gradient.store.register %gradient.store.sum.wide = load float, ptr addrspace(5) %gradient.store.sum.ptr, align 4 %gradient.store.sum = call float @recipe.encode(float %gradient.store.sum.wide) store float %gradient.store.sum, ptr addrspace(1) %gradient.store.ptr, align 4
