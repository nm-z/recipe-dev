...
entry:
%chunk.sums = alloca [RECIPE_CHUNK_VALUES x double], align 8, addrspace(5)
...
local.k.begin:
%local.sums.initial = load <RECIPE_REGISTER_COUNT x double>, ptr addrspace(5) %sums, align 8
...
local.store:
store <RECIPE_REGISTER_COUNT x double> %local.sums.current, ptr addrspace(5) %sums, align 8
br label %exit
shared.chunk.loop:
...
%zero.ptr = getelementptr double, ptr addrspace(5) %chunk.sums, i32 %zero.index
store double %state.zero, ptr addrspace(5) %zero.ptr, align 8
...
%sum.ptr = getelementptr double, ptr addrspace(5) %chunk.sums, i32 %sum.index
...
%publish.source = getelementptr double, ptr addrspace(5) %chunk.sums, i32 %publish.source.index
%publish.value = load double, ptr addrspace(5) %publish.source, align 8
...
%fold.target = getelementptr double, ptr addrspace(5) %sums, i32 %fold.r
%fold.current = load double, ptr addrspace(5) %fold.target, align 8
%fold.next = call double @recipe.state.add(double %fold.current, double %fold.value)
store double %fold.next, ptr addrspace(5) %fold.target, align 8
...
channel.begin:
%sum.ptr = getelementptr [RECIPE_REGISTER_N x double], ptr addrspace(5) %sums, i32 0, i32 %slot
%previous = load double, ptr addrspace(5) %sum.ptr, align 8 %initial = select i1 %first, double %zero, double %previous br label %r.loop
r.loop:
...
sum.store:
store double %sum, ptr addrspace(5) %sum.ptr, align 8 br i1 %last, label %destination.store, label %channel.done
destination.store:
...
%matrix.first.target = getelementptr double, ptr addrspace(5) %sums, i32 %matrix.register
%matrix.first.current = load double, ptr addrspace(5) %matrix.first.target, align 8
%matrix.first.sum = call double @recipe.state.add(double %matrix.first.current, double %matrix.first.value)
store double %matrix.first.sum, ptr addrspace(5) %matrix.first.target, align 8
...
%matrix.second.target = getelementptr double, ptr addrspace(5) %sums, i32 %matrix.second.register
%matrix.second.current = load double, ptr addrspace(5) %matrix.second.target, align 8
%matrix.second.sum = call double @recipe.state.add(double %matrix.second.current, double %matrix.second.value)
store double %matrix.second.sum, ptr addrspace(5) %matrix.second.target, align 8
...
%matrix.third.target = getelementptr double, ptr addrspace(5) %sums, i32 %matrix.third.register
%matrix.third.current = load double, ptr addrspace(5) %matrix.third.target, align 8
%matrix.third.sum = call double @recipe.state.add(double %matrix.third.current, double %matrix.third.value)
store double %matrix.third.sum, ptr addrspace(5) %matrix.third.target, align 8
...
%matrix.fourth.target = getelementptr double, ptr addrspace(5) %sums, i32 %matrix.fourth.register
%matrix.fourth.current = load double, ptr addrspace(5) %matrix.fourth.target, align 8
%matrix.fourth.sum = call double @recipe.state.add(double %matrix.fourth.current, double %matrix.fourth.value)
store double %matrix.fourth.sum, ptr addrspace(5) %matrix.fourth.target, align 8
...
%sums = alloca [RECIPE_REGISTER_COUNT x double], align 8, addrspace(5) %state.zero = call double @recipe.state.from.u1(i1 false) %lid = call i32 @llvm.amdgcn.workitem.id.x() %group = call i32 @llvm.amdgcn.workgroup.id.x() %block = call i32 @recipe.workgroup.size.x() %groups = udiv i32 %threads, %block
...
sum.init.step: %sum.init.ptr = getelementptr [RECIPE_REGISTER_COUNT x double], ptr addrspace(5) %sums, i32 0, i32 %sum.init store double %state.zero, ptr addrspace(5) %sum.init.ptr, align 8 %sum.init.next = add i32 %sum.init, 1 br label %sum.init.loop sum.init.done: br label %tile.loop tile.loop:
...
%sums = alloca [RECIPE_REGISTER_COUNT x double], align 8, addrspace(5)
%bias.sums = alloca [RECIPE_REGISTER_N x double], align 8, addrspace(5)
%state.zero = call double @recipe.state.from.u1(i1 false) %lid = call i32 @llvm.amdgcn.workitem.id.x() %group = call i32 @llvm.amdgcn.workgroup.id.x() %block = call i32 @recipe.workgroup.size.x() %groups = udiv i32 %threads, %block
...
%gradient.store.ptr = getelementptr inbounds double, ptr addrspace(1) %gradient.destination, i32 %gradient.store.index %gradient.store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x double], ptr addrspace(5) %sums, i32 0, i32 %gradient.store.register %gradient.store.sum.wide = load double, ptr addrspace(5) %gradient.store.sum.ptr, align 8 %gradient.store.sum = call double @recipe.encode(double %gradient.store.sum.wide) store double %gradient.store.sum, ptr addrspace(1) %gradient.store.ptr, align 8
...
previous.sum.init.step: %previous.sum.init.ptr = getelementptr [RECIPE_REGISTER_COUNT x double], ptr addrspace(5) %sums, i32 0, i32 %previous.sum.init store double %state.zero, ptr addrspace(5) %previous.sum.init.ptr, align 8 %previous.sum.init.next = add i32 %previous.sum.init, 1 br label %previous.sum.init.loop previous.tile.loop:
...
%previous.store.old = load double, ptr addrspace(1) %previous.store.ptr, align 8 %previous.store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x double], ptr addrspace(5) %sums, i32 0, i32 %previous.store.register %previous.store.sum.wide = load double, ptr addrspace(5) %previous.store.sum.ptr, align 8 %previous.store.sum = call double @recipe.encode(double %previous.store.sum.wide) %previous.store.value = call double @recipe.add(double %previous.store.old, double %previous.store.sum) store double %previous.store.value, ptr addrspace(1) %previous.store.ptr, align 8 br label %previous.store.next
