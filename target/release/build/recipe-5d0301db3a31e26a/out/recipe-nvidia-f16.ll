...
declare <8 x float> @recipe.wmma(<16 x half>, <16 x half>, <8 x float>)
...
%store.bias.base = mul i32 %out.channels, %terms %store.bias.index = add i32 %store.bias.base, %store.channel %store.bias.ptr = getelementptr inbounds half, ptr addrspace(1) %weights, i32 %store.bias.index %store.bias = load half, ptr addrspace(1) %store.bias.ptr, align 2 %store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr %sums, i32 0, i32 %store.register %store.sum.wide = load float, ptr %store.sum.ptr, align 4 %store.sum = call half @recipe.encode(float %store.sum.wide)
