...
declare <8 x float> @recipe.wmma(<16 x i8>, <16 x i8>, <8 x float>)
...
%gradient.store.ptr = getelementptr inbounds i8, ptr addrspace(1) %gradient.destination, i32 %gradient.store.index %gradient.store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr %sums, i32 0, i32 %gradient.store.register %gradient.store.sum.wide = load float, ptr %gradient.store.sum.ptr, align 4 %gradient.store.sum = call i8 @recipe.encode(float %gradient.store.sum.wide) store i8 %gradient.store.sum, ptr addrspace(1) %gradient.store.ptr, align 1
