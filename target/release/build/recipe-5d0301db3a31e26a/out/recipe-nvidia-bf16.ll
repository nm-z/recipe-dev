...
declare <8 x float> @recipe.wmma(<16 x i16>, <16 x i16>, <8 x float>)
...
%store.bias.base = mul i32 %out.channels, %terms %store.bias.index = add i32 %store.bias.base, %store.channel %store.bias.ptr = getelementptr inbounds i16, ptr addrspace(1) %weights, i32 %store.bias.index %store.bias = load i16, ptr addrspace(1) %store.bias.ptr, align 2 %store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr %sums, i32 0, i32 %store.register %store.sum.wide = load float, ptr %store.sum.ptr, align 4 %store.sum = call i16 @recipe.encode(float %store.sum.wide)
...
%gradient.store.ptr = getelementptr inbounds i16, ptr addrspace(1) %gradient.destination, i32 %gradient.store.index %gradient.store.sum.ptr = getelementptr [RECIPE_REGISTER_COUNT x float], ptr %sums, i32 0, i32 %gradient.store.register %gradient.store.sum.wide = load float, ptr %gradient.store.sum.ptr, align 4 %gradient.store.sum = call i16 @recipe.encode(float %gradient.store.sum.wide) store i16 %gradient.store.sum, ptr addrspace(1) %gradient.store.ptr, align 2
