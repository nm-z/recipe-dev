...
fn main() {
    let data = recipe.gguf(GGUF).fp(32);
    ...
    let mut model = recipe.model().no(bias)
        .epsilon(float("gemma3.attention.layer_norm_rms_epsilon"))
        .embed(embedding.shape[1] as usize, width).scale((width as f64).sqrt());

    for layer_index in 0..integer("gemma3.block_count") {
        model = model.res([
            norm(rms),
            attn(heads).kv(kv).width(head).qk(rms)
                .rope(neox, head, float("gemma3.rope.freq_base"))
                .yarn(float("gemma3.rope.scaling.factor"), integer("gemma3.rope.scaling.original_context_length"),
                    float("gemma3.rope.scaling.yarn_beta_fast"), float("gemma3.rope.scaling.yarn_beta_slow")),
            norm(rms),
        ]).res([
            norm(rms),
            recipe.model().no(bias).layer(integer("gemma3.feed_forward_length")).gelu()
                * recipe.model().no(bias).layer(integer("gemma3.feed_forward_length")),
            layer(width),
            norm(rms),
        ]);
        let tensor = |name: &str| data.tensor(&format!("blk.{layer_index}.{name}.weight")).unwrap().clone();
        let qk = std::iter::repeat_n(tensor("attn_q_norm"), heads)
            .chain(std::iter::repeat_n(tensor("attn_k_norm"), kv)).collect::<Vec<_>>();
        plan = plan.node(&[tensor("attn_norm")])
            .node(&[tensor("attn_q"), tensor("attn_k"), tensor("attn_v")]).node(&qk)
            .node(&[tensor("attn_output")]).node(&[tensor("post_attention_norm")])
            .node(&[tensor("ffn_norm")]).node(&[tensor("ffn_gate")]).node(&[tensor("ffn_up")])
            .node(&[tensor("ffn_down")]).node(&[tensor("post_ffw_norm")]);
    }
    let cap = float("gemma3.final_logit_softcapping");
    model = model.norm(rms).layer(embedding.shape[1] as usize).scale(1.0 / cap).tanh().scale(cap);
    ...
    let stop = tokenizer.encode("<|eot_id|>").into_iter().filter(|id| *id != 128000).collect::<Vec<_>>();
    ...
    eprintln!("prefill {} s; {} decode steps in {} s ({} tok/s)", generated.prefill_seconds,
        generated.step_seconds.len(), seconds, generated.step_seconds.len() as f64 / seconds);
}
