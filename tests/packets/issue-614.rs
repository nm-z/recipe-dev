...
fn main() {
	...
	let bundle = directory.join("issue-614.ogdl");
	...
	let data = recipe.data("data/categoric/single_csv.csv")
		.target("target");
	let model = recipe.model()
		.knn(3).leak().norm(batch).qi(8).0
		.forest(2).huber().norm(batch).qi(4).1
		.loss(bce);
