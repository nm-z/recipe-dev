...
fn build(case: &Case) -> Model {
	match case.shape {
		...
		"attention-rope" => recipe.model().conv(4, 3).attn(2).rope(neox, 2, 10000.0).relu().layer(1).loss(mse),
		"attention-sparse" => recipe.model().conv(8, 3).attn(4).kv(2).qk(rms).rope(neox, 2, 10000.0).index(1, 2, 4, 2).gate().relu().layer(1).loss(mse),
