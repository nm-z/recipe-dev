...
gemma4 for contexts through its sliding window, qwen2/qwen3/qwen3moe,
qwen3.5/qwen3next, qwen4, and lfm2. Missing and needed: phi, mistral,
...
are dated observations, not measurements of the current tree. By implemented
contract breadth, the tree is now roughly halfway to this document. Acceptance is
stricter: only 2 of the 8 first-milestone items are fully complete.
...
| Model definition | The public suffixes are `.fp`, `.int(4|8|16|32)`, `.bf`, and `.tf`. `.int(1)`, `.f()`, `.qi()`, `.iq()`, `.quantize()`, `.profile()`, and `.step()` are removed from `Block` and `Model`; build templates and new bundle precision tokens no longer contain int1 or arbitrary floats. Distinct fields and lowering scopes now keep primary ops, activations, output norms, Q/K norms, RoPE, KV cache, depthwise convolution, and residual suffixes local, so a later suffix cannot overwrite an earlier op. The table supplies every node's step. Model and product branches no longer carry run-level storage selection, and new bundles write a `model2` header with no quantization field. | Legacy storage codecs and the former model, product, block, and step fields remain read adapters only. Extend the same one-op ownership to future attention indexer modifiers when they gain public precision suffixes. |
| Organization and tracing | Precision, layout, node identities, traces, and reference comparisons exist. Primary, activation, norm, Q/K, RoPE, and KV precision fields are distinct. `Node.storage` is also distinct from the nine structural arguments, which removed the RoPE/storage alias exposed by Gemma 4. Plain architecture residuals are real `Residual` blocks, so placement can cut them. The centralized value allocator has a reuse diagnostic; disabling reuse removed Gemma 4's range-size-dependent nonfinite output for 54 MB on a 7.9 GB range, so the conservative layout is now default. | Repair and prove retained-output lifetime before making `RECIPE_VALUE_REUSE=1` the default. Consolidate the remaining duplicated schedule and kernel-interface decisions, preserve operation identity through every fusion, and verify other buffer ownership and lifetime rules. |
...
| Speed | Historical session reports: rnj-1 decode at 25-33 tok/s on the 7700 XT. A current uncached M60 build produced 5.05 MB LLVM IR and took 162.5 s through compile/JIT; the cached run prepared in 4.69 s and decoded at 3.79 tok/s. A later uncached `llamacpp`-profile variant prepared in 238.9 s, then prefetched in 16.54 s and decoded at 2.13 tok/s. The same profile on `amd0` prepared in 68.15 s, prefetched in 6.48 s, and decoded one token in 0.088 s. Direct Clang-to-PTX also exceeded 240 s. | Emit repeated layers as loops, split variants into parallel modules, and remeasure the current tree with recorded model, format, context, and device. Measure prefill, establish ceilings from actual resident bytes and verified bandwidth, and meet the decode and compile targets. |
| Correctness | Reference recording and comparison are wired into both decode paths. Bounded CPU/M60 fixtures pass tolerance checks; the CPU fixture also passes exact comparison under the `llamacpp` policy. The NVIDIA attention argument-width defect is fixed and verified. Full `llamacpp`-profile runs complete on both the M60 and `amd0` without a kernel deadlock. The corrupt chained-RoPE output was reproduced and traced to Q/K normalization and RoPE sharing one fp16 suffix slot; separate slots and explicit fp32 RoPE restore coherent 32-layer output. | Verify full-model logits against llama.cpp and fix the remaining token mismatch. Changing YaRN fast from the GGUF's 64 to llama.cpp's Gemma3 default of 32 changes the result materially, while step 32 versus 256 does not repair the first-token mismatch. Online softmax alone remains coherent over all 32 layers, and one-layer checks keep every isolated llama-order variant within tolerance. An exact comparison policy is not proof of llama.cpp parity. |
...
| Several cards | Layer placement exists. Two-M60 data-parallel training verifies gradient aggregation and weight synchronization on a bounded fixture. A 12B Gemma 4 graph completed finite inference as `[55,45]` over two M60s, `[25,25,25,25]` over four, and `[17,17,17,17,16,16]` over six. Resident bytes and per-token movement are recorded. The fitter backtracks to a legal residual boundary instead of failing at an illegal overflow boundary. | The inference tokens were wrong, and this does not establish model-parallel training or a 27B model. Verify logits, training, and measured hop costs. The pageable host-transfer path still needs a current audit and the planned pinned DMA/P2P work. |
| Architectures | Builder entries exist for llama, gemma3, gemma4, qwen2/3 families, qwen3.5/next, qwen4, and lfm2. LFM2.5 produced the same greedy answer as llama.cpp on a complete 1.2B Q8_0 model on an M60, and its tokenizer IDs match. Gemma 4 binds all 48 layers and 667 tensors, including varying attention geometry, optional V=K, proportional RoPE, post norms, output scales, softcap, suppress tokens, and its exact tokenizer. Two-, four-, and six-M60 short-context decodes complete but produce wrong tokens. | Fix Gemma 4 logits and safely enable its raw QK attention scale, then add the sliding mask for contexts above 1,024 and macro/tool chat-template coverage. Verify LFM2 logits within tolerance and add the other missing families. A bound graph or matching answer alone does not establish full correctness. |
| Chat | The server constructs a full conversation prompt and accepts a requested reply budget. Its Rust worker places the script-defined GGUF model once, reads requests over one pipe, reuses the same `Placed` tapes, and reports the device, resolved startup routes, prefill time, decode time, and tok/s under each reply. One M60 process answered two direct requests after one `READY`, then the HTTP path increased from 37 to 51 prompt tokens when the second request included the prior turn. Server termination also stops the resident process group. | Verify browser-local persistence and the visual rendering of per-reply statistics. A stopped browser request currently lets the in-flight device decode finish before the server accepts another request. |
| Measurement tools | `recipe stats <file.gguf>` streams rows through the loader's decoders and prints per-tensor mean absolute weight, RMS, participation, three-sigma RMS outlier rows, and a normalized histogram. A mixed Q8_0/F32 fixture verifies both block and float decoding, and a full 1.2B LFM2.5 run streamed all 148 tensors. A 6.5 GB Gemma 4 pass stayed below 1 GB RSS while checking real F32, Q4_0, and Q6_K tensors. Learned LFM projections clustered around p=0.58–0.63, while short-convolution taps were about 0.42. | Cross-check representative Q2/Q3/Q5/IQ/NF4 files against the earlier scripts, then add stable machine-readable output if downstream tools need it. |
...
- [RNJ chained-RoPE precision diagnosis](/home/nate/codex/rnj-rope-diagnosis-20260919/RESULTS.md).
- [Gemma 4 builder and six-M60 placement verification](/home/nate/codex/gemma4-verification-20260919/RESULTS.md).
- [LFM2.5 full-file statistics verification](/home/nate/codex/lfm-stats-20260919/RESULTS.md).
- [Gemma 4 Q4_0/Q6_K/F32 statistics verification](/home/nate/codex/gemma4-stats-20260919/RESULTS.md).
...
4. **Partial:** Chained RoPE was the reproduced source of corrupt 32-layer text:
