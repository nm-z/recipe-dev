...
| Organization and tracing | Precision, layout, node identities, traces, and reference comparisons exist. Primary, activation, norm, Q/K, RoPE, and KV precision fields are distinct. `Node.storage` is also distinct from the nine structural arguments, which removed the RoPE/storage alias exposed by Gemma 4. Plain architecture residuals are real `Residual` blocks, so placement can cut them. | Consolidate the remaining duplicated schedule and kernel-interface decisions, preserve operation identity through every fusion, and verify buffer ownership and lifetime. The Gemma 4 two-range nonfinite result shows that range-size-dependent execution still needs tracing. |
...
| Several cards | Layer placement exists. Two-M60 data-parallel training verifies gradient aggregation and weight synchronization on a bounded fixture. A 12B Gemma 4 graph also loaded explicit `[17,17,17,17,16,16]` ranges across six M60s and completed a finite decode. The fitter now backtracks to a legal residual boundary instead of failing at an illegal overflow boundary. | The six-card inference tokens were wrong, and this does not establish model-parallel training or a 27B model. Verify logits, training, and measured hop costs. The pageable host-transfer path still needs a current audit and the planned pinned DMA/P2P work. |
| Architectures | Builder entries exist for llama, gemma3, gemma4, qwen2/3 families, qwen3.5/next, qwen4, and lfm2. LFM2.5 produced the same greedy answer as llama.cpp on a complete 1.2B Q8_0 model on an M60, and its tokenizer IDs match. Gemma 4 binds all 48 layers and 667 tensors, including varying attention geometry, optional V=K, proportional RoPE, post norms, output scales, softcap, and its exact tokenizer. A six-M60 short-context decode completes but produces wrong tokens. | Fix Gemma 4 logits and the range-size-dependent nonfinite result, then add the sliding mask for contexts above 1,024 and macro/tool chat-template coverage. Verify LFM2 logits within tolerance and add the other missing families. A bound graph or matching answer alone does not establish full correctness. |
